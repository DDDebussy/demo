...
public class CustomImageCache implements ImageLoader.ImageCache {
    ...
    public CustomImageCache() {
        ...
        try {
            // 获取图片缓存路径
            File cacheDir = CommonUtils.getDiskCacheDir(GlobalContext.getInstance(), "thumb");
            if (!cacheDir.exists()) {
                cacheDir.mkdirs();
            }
            // 创建DiskLruCache实例，初始化缓存数据
            mDiskLruCache = DiskLruCache
                    .open(cacheDir, CommonUtils.getAppVersion(GlobalContext.getInstance()), 1, DISK_CACHE_MAX_SIZE);
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
